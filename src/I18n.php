...
class I18n
{
    ...
    private static function loadNls(): void
    {
        if (!is_array(self::$nls)) {
            ...
            $nlsdir = __DIR__ . DIRECTORY_SEPARATOR;
     ...
     *
     * @return Nls|false first suitable lang object or false
     */
     ...
     *
     * @return Nls suitable lang object
     */
    public static function getLanguage(array $availableLangs, string $defaultLang)
    {
        ...
        if (preg_match('%^/([a-z]{2})/?[\w\-_/]*$%', $_SERVER['REQUEST_URI'], $regs)) {
            ...
            if (!$i18n) {
                $i18n = Nls::init($defaultLang);
            } elseif (!in_array($i18n->key(), $availableLangs, true)) {
                $i18n = Nls::init($defaultLang);
            }
        } elseif (isset($_SESSION['language'])) {
            $i18n = Nls::init($_SESSION['language']);
        } else {
